#ifndef STORAGE_LEVELDB_UTIL_SIMPLEQUEUE_H_
#define STORAGE_LEVELDB_UTIL_SIMPLEQUEUE_H_
...
class SimpleQueue {
 public:
  ...
  // true.
  bool Pop(T* value) {
    std::unique_lock<std::mutex> ml_(mutex_);
    while (queue_.size() == 0 && !no_more_jobs_) cv_.wait(ml_);
  ...
  void Push(const T& value) {
    {
      std::lock_guard<std::mutex> ml_(mutex_);
...
#endif  // STORAGE_LEVELDB_UTIL_SIMPLEQUEUE_H_